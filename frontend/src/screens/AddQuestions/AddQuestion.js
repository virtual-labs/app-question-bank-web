...
function Form({token,uEmail,tags,setTags}) {
	// console.log(uEmail);
	...
	const handleClearSubmit = async () => {
		...
		function proceedWithData() {
			...
			const formData = constructFormData(CorrectOption, Difficulty, selectedtags, image_url,uEmail);
	...
	return (
		...
		>
			...
			<div>
				...
				{image ? (
					<img id="selected-image" src={URL.createObjectURL(image)} alt="Selected" style={{
						maxWidth: '100%',
						maxHeight: '200px',
						marginTop: '10px',
					}} />
...
// 	);
// }
const constructFormData = (CorrectOption, Difficulty, selectedtags, imageUrl,uEmail) => {
...
function AddToQueueSharption({ setquestions, setDownloadList, token, setToken, selectedRole, setSelectedRole ,tags,setTags}) {
	// setquestions([])
		...
		<div>
			...
			<Card sx={{ margin: 'auto', maxWidth: 600, padding: 2 }}>
				<CardContent>
					...
					<Form tags={tags} setTags={setTags} token={token} uEmail={userEmail}/>
				</CardContent>
			</Card>
		</div>
	);
